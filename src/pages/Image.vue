    ...
    <div class="q-pa-md q-gutter-md">
      <q-card class="q-ma-none" transition-show="slide-left">
        <q-item class="bg-orange-3">
          ...
          <q-item-section side>
            <q-btn size="12px" flat dense round icon="mdi-close" @click="showChecklist"> 
            </q-btn>
          </q-item-section>
        </q-item>
...
export default {
  ...
  data () {
    return {
      ...
      imageSource: '',
      serverSession: SessionStorage.getItem('sessionKey'),
  ...
  mounted () {
    ...
    xhttp.onload = () => {
      ...
      this.imageSource = resultObj.imageSource;
      ...
      if (resultCode != 0) {
        ...
        //console.log(decodeURI(this.imageSource));
        var fileExt = this.imageSource.substr(this.imageSource.lastIndexOf('.') + 1);
        if (fileExt == 'pdf') {
          ...
          this.isPdf = false;
          this.isImage = true;
          document.getElementById('downloadedImage').src = this.imageSource;
        }
      }
    }
    ...
    xhttp.setRequestHeader('CheckListId', SessionStorage.getItem('checklistId'));
