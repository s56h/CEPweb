...
<!--                </div>
        ...
        <q-carousel-slide name="checklist" class="column no-wrap q-mt-xl"> -->
          <q-chip v-if="hasMandChecklist" style="margin-top: 70px" outline square color="deep-orange" text-color="white" icon="mdi-alert-outline">
            <b>Mandatory documents:&nbsp;</b> {{ mandText }}
          </q-chip>
          <q-list v-if="hasMandChecklist" class="q-ml-none" style="margin-top: 6px">
            <q-item class="q-px-xs" v-for="item in installerMandChecklist" :key="item.rowId">
              <q-item-section class="col-1" top>
                <q-checkbox :model-value="item.cb" :color="item.cbCol" />
              </q-item-section>
              <q-item-section top class="col-7 q-py-xs">
                <q-item-label class="q-pt-xs">
                  <div class="q-pt-xs text-grey-9 text-bold">{{ item.checkName }}</div> <!-- temp -->
                  <div :class="item.txtCol" class="q-ml-md">{{ item.textLine1 }}</div>
                  <div class="text-weight-medium q-ml-md">{{ item.textLine2 }}</div>
                </q-item-label>
              </q-item-section>
          ...
          <q-chip v-if="hasOtherChecklist" class="q-mt-sm" outline square color="primary" text-color="white" icon="mdi-alert-circle-outline">
            <b>Optional documents:&nbsp;</b> {{ otherText }}
          </q-chip>
          <q-list v-if="hasOtherChecklist" class="q-ml-none" style="margin-top: 6px">
            <q-item class="q-px-xs" v-for="item in installerOtherChecklist" :key="item.rowId">
              <q-item-section class="col-1" top>
                <q-checkbox :model-value="item.cb" :color="item.cbCol" />
              </q-item-section>
              <q-item-section top class="col-7 q-py-xs">
...
-->
      <q-page-sticky expand position="top" style="width: 100%" :offset="[0, 0]">    <!-- Carousel page title -->
        <q-toolbar class="bg-blue-5 text-white">
          ...
          <q-toolbar-title v-if="mainName === 'installerlist'"><span>Invoice List</span></q-toolbar-title>
          <q-toolbar-title v-if="mainName === 'checklist'">Document Checklist</q-toolbar-title>
        </q-toolbar>
      </q-page-sticky>
...
<script>
//v1.2 import { useRoute } from 'vue-router';
//v1.2 import { watch } from 'vue';
import { ref } from 'vue';
...
export default {
  ...
  data () {
    ...
    return {
      installerMandChecklist: new Array(),
      installerChecklist: new Array(),
      installerOtherChecklist: new Array(),
      installerInvoiceList: new Array(),
      projectList: new Array(),
      checklistMandOk: false,
      dateNeeded: true,
      ...
      hasChecklist: false,
      hasMandChecklist: false,
      mandOkCount: 0,
      mandText: 'x of y submitted',
      hasOtherChecklist: false,
      otherOkCount: 0,
      otherText: 'x of y submitted',
      hasInvoices: false,
      hasWorkorders: false,
      mainName: 'checklist',
  ...
  watch: {
    mainName(newPage) {
      if (newPage == 'submitinvoice') {
        if (!this.checklistMandOk) {
          this.resultMsg = 'Your "needed to start work" documents are not complete. Please load them as soon as possible. Contact CCC if you have questions.';  // **********
          this.resultType = 'Error';
          this.resultTitle = 'Mandatory Items Needed';
  ...
  created () {
    ...
    .then(response => {
      ...
      if (postResponse.resultCode != 0) {
        ...
        this.installerChecklist = this.setCheckList(postResponse.checkList);
        var displayLists = this.setDisplayLists(this.installerChecklist);
        this.installerMandChecklist = displayLists.installerMandChecklist;
        this.installerOtherChecklist = displayLists.installerOtherChecklist;
        //console.log('Results:');
        //console.log(this.installerChecklist);
        //console.log('Mand:');
        //console.log(this.installerMandChecklist);
        //console.log('Other:');
        //console.log(this.installerOtherChecklist);
        //this.checklistPLok = setCheckListResult.checklistPLok;
        //v1.2 this.installerInvoiceList = this.setInvoiceList(postResponse.invoiceList);
        //v1.2 this.projectSelect = this.setProjectSelect(postResponse.projectList);
        //v1.2 this.statementData = postResponse.statementData;
        //v1.2 this.$router.replace('/app/home/invoicelist');
...
*/
    ...
    setCheckList (resultList) {
      ...
      for (let i = 0; i < resultList.length; i++) {
        checkId = resultList[i].Item.Id;
        fileName = resultList[i].Item.File;
        expiryDateStr = resultList[i].Item.Exp;
        checkName = resultList[i].Item.Name;
        hasExpiry = resultList[i].Item.ExpFlag;
        documentStatus = resultList[i].Item.Status;
        mandFlag = resultList[i].Item.MandFlag;
        checklistType = resultList[i].Item.Type;
        if (fileName == null)
          docExists = false;
        else
          docExists = true;
        ...
        if (documentStatus == 'Active' || documentStatus == 'Pending') {
          if (!hasExpiry) {
            ...
            if (expiryDate > today) {
              ...
              triggerDate.setDate(triggerDate.getDate() + 21); // 21 days is set as threshold for about to expire warning
        ...
        var checkItem = {rowId: i, id: checkId, cb: cbTick, checkName: checkName, mandFlag: mandFlag, textLine1: textLine1, textLine2: textLine2, hasExpiry: hasExpiry, expiryDate: expiryDate, documentStatus: documentStatus, cbCol: cbColour, txtCol: textColour, exists: docExists, fileName: fileName, type: checklistType};
        //console.log(checkItem);
        checkList.push(checkItem);
      }
      SessionStorage.set('checkList',checkList);    //  needed for upload
      //console.log(checkList);
      return checkList;
    },

    setDisplayLists(checklistData) {
      //
      //      Set lists that drive the Document Checklist
      //
      var installerMandChecklist = new Array();
      var installerOtherChecklist = new Array();
      var mandOkCount = 0;
      var otherOkCount = 0;
      for (let i = 0; i < checklistData.length; i++) {
        if (checklistData[i].mandFlag == 1)
        {
          installerMandChecklist.push(checklistData[i]);
          this.hasMandChecklist = true;
          if (checklistData[i].cb) mandOkCount += 1;
        }
        else {
          installerOtherChecklist.push(checklistData[i]);
          this.hasOtherChecklist = true;
          if (checklistData[i].cb) otherOkCount += 1;
        }
      }
      if (mandOkCount == installerMandChecklist.length)
        this.mandText = 'all ready';
      else
        this.mandText = mandOkCount + ' of ' + installerMandChecklist.length + ' ready';
      if (otherOkCount == installerOtherChecklist.length)
        this.otherText = 'all ready';
      else
        this.otherText = otherOkCount + ' of ' + installerOtherChecklist.length + ' ready';
      return {installerMandChecklist: installerMandChecklist, installerOtherChecklist: installerOtherChecklist};
    },
    ...
    documentLook(rowId) {
      ...
      SessionStorage.set('fileName',this.installerChecklist[rowId].fileName);
    ...
    doDelete() {
      ...
      axios.get('Services/DeleteDocument.aspx', {
        headers: {
          'Content-Type': 'multipart/form-data',
          'UserEmail': SessionStorage.getItem('userEmail'),
          'InstallerId': SessionStorage.getItem('installerId'),
