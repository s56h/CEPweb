...
<%
	...
	Else
		Try
			...
			sqlChecklistCmd.CommandText = "SELECT C.ChecklistID, C.FileFolderPath, C.ExpiryDate, L.Name AS CheckName, L.HasExpiryDate, S.Name AS Status, 'I' AS Type FROM tbl_installer_checklist C, tbl_checklist_status S, tbl_checklist L WHERE (C.InstallerID = " & strInstallerID & ") AND (C.CheckListID = L.ID) AND (C.ChecklistStatusID = S.ID) "
			...
			End If
			sqlChecklistCmd.CommandText = sqlChecklistCmd.CommandText & " ORDER BY L.Name FOR JSON AUTO, INCLUDE_NULL_VALUES"
		...
		Catch ex As Exception
			...
			strResultMsg = "There was an error while retrieving your data. Please contact Carpet Cutters Commercial."
			strResultType = "System Error"
			PutLog("CEP app", "Installer", strUserEmail, strResultType, "GetInstallerLists", strResultMsg & " Code: " & intResultCode & " " & ex.Message)
