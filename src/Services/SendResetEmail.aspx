...
<%
    ...
    Try
        ...
        If sqlReader.HasRows Then		'	the user email is correct
			...
			If intUpdateCount = 1 Then		'	the reset key has been updated, send email
				...
				Dim strEmailLogTypeId As String
				strEmailLogTypeId = oAppSettings("EmailLogTypeId")
				...
				sqlUpdateCmd2.CommandText = "INSERT INTO tbl_email_log (EmailLogTypeID, ToEmail, FromEmail, CC, Bcc, EmailSubject, EmailBody, CreatedDateTime) VALUES (" & strEmailLogTypeId & ", '" & strUserEmail & "', 'noreply@carpetcutters.com.au', '', '', '" & strMailSubject & "', '" & strEmailBody & "', GETDATE())"
				PutLog("CEP app", "Installer", strUserEmail, "Debug", "SendResetEmail", "SQL: " & sqlUpdateCmd2.CommandText)
				intUpdateCount = sqlUpdateCmd2.ExecuteNonQuery()

				If intUpdateCount = 0 Then
					intResultCode = 90
					strResultTitle = "Email Problem"
					strResultMsg = "There was an unexpected error sending your email (91). Please contact Carpet Cutters Commercial."
					strResultType = "System Error"
					PutLog("CEP app", "Installer", strUserEmail, strResultType, "SendResetEmail", strResultMsg & " Unable to insert into tbl_email_log.")
				Else
					intResultCode = 10
					strResultTitle = "Email sent"
					strResultMsg = "An email has been sent to allow you to enter a new password"
	...
	Response.Clear()	'	Previous code somewhere (stored procedure call?) puts spurious data in Response
	...
	PutLog("CEP app", "Installer", strUserEmail, "Debug", "SendResetEmail", "Ended with " & intResultCode.ToString())
